...
template <typename T>
void ToolBox::TrieNode<T>::display(std::ostream &os, unsigned offset, unsigned char label, std::string& buffer) const
{
  if (label)
      buffer += label;
  if(_value > 0)
      os << std::setw(29) << std::left << buffer << std::setw(16) << std::left  << _value << std::endl;
  if (_firstSubNode)
    _firstSubNode->display(os, offset + 2, _firstSubNodeLabel, buffer);
  buffer = buffer.substr(0, buffer.size()-1);
  if (_brother)
    _brother->display(os, offset, _brotherLabel, buffer);
}
